...
public class AuthServiceImpl implements AuthService {
   ...
   // 1. TẠO HÀM CHUNG: Chỉ kiểm tra Email, Trạng thái và Mật khẩu (Không check Role)
    private Users validateUserCredentials(String email, String password) {
    ...
    // 2. HÀM CHO CLIENT (Khách hàng)
    @Override
    public Users Login(String email, String password) {
        // Gọi hàm chung để check mật khẩu
        Users user = validateUserCredentials(email, password);

        // Kiểm tra Role: Chặn Admin không cho đăng nhập ở màn hình Client
        String roleName = user.getRole().getName();
        if ("ADMIN".equalsIgnoreCase(roleName) || "ROLE_ADMIN".equalsIgnoreCase(roleName)) {
            throw new RuntimeException("Tài khoản quản trị không được phép đăng nhập tại khu vực khách hàng.");
        }

        return user;
    }

    // 3. HÀM CHO ADMIN (Quản trị viên)
    @Override
    public Users adminLogin(String email, String password) {
        // Gọi hàm chung để check mật khẩu
        Users user = validateUserCredentials(email, password);

        // Kiểm tra Role: Bắt buộc phải là Admin
        String roleName = user.getRole().getName();
        if (roleName == null || (!roleName.equalsIgnoreCase("ADMIN") && !roleName.equalsIgnoreCase("ROLE_ADMIN"))) {
            throw new RuntimeException("Truy cập bị từ chối! Tài khoản không có quyền quản trị.");
        }
